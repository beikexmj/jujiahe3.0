...
#define ViewW (self.frame.size.width - 40)     //当前视图宽度
...
- (void)setSelectDateArray:(NSArray*)selectDateArray{
    if (_selectDateArray != selectDateArray) {
        _selectDateArray = selectDateArray;
    }
    if (_middleView) {
        _middleView.selectDateArray = selectDateArray;
        _middleView.continueSignDateArray = _continueSignDateArray;
        [_middleView.collectionV reloadData];
    }
}
...
+ (CGFloat)getMonthTotalHeight:(NSDate *)date type:(CalendarType)type {
    if (type == CalendarType_Week) {
        return yearMonthH*1.5 + weeksH + dayCellH;
    } else {
        NSInteger rows = [[YXDateHelpObject manager] getRows:date];
        return yearMonthH*1.5 + weeksH + rows * dayCellH;
    }
...
- (void)settingHeadLabel {
    ...
    _yearMonthL = [[UILabel alloc] initWithFrame:CGRectMake(20, 0, ViewW, yearMonthH)];
    _yearMonthL.text = [[YXDateHelpObject manager] getStrFromDateFormat:@"MM月签到表" Date:_currentDate];
    _yearMonthL.textAlignment = NSTextAlignmentLeft;
    _yearMonthL.font = [UIFont systemFontOfSize:13];
    _yearMonthL.textColor = [UIColor colorWithRed:96/255.0 green:96/255.0 blue:96/255.0 alpha:1];
//    [self addSubview:_yearMonthL];
    
    NSArray *weekdays = @[@"日",@"一",@"二",@"三",@"四",@"五",@"六"];
    CGFloat weekdayW = ViewW/7;
    UIView *weekNameView = [[UIView alloc]initWithFrame:CGRectMake(20, yearMonthH, ViewW, weeksH)];
    weekNameView.backgroundColor = [UIColor colorWithRed:233/255.0 green:241/255.0 blue:248/255.0 alpha:1];
    weekNameView.layer.cornerRadius = 5;
    weekNameView.layer.masksToBounds = YES;
    ...
    for (int i = 0; i < 7; i++) {
        ...
        weekL.font = [UIFont systemFontOfSize:13];
        weekL.backgroundColor = [UIColor clearColor];
        weekL.text = weekdays[i];
        [weekNameView addSubview:weekL];
    }
...
- (void)settingScrollView {
    ...
    _scrollV = [[UIScrollView alloc] initWithFrame:CGRectMake(20, yearMonthH*1.5 + weeksH, ViewW, ViewH - yearMonthH*1.5 - weeksH)];
