...
@interface LifeVC ()<WKNavigationDelegate, WKUIDelegate>
{
    EmptyView *myView;
}
...
- (void)configureWKWebview{
    ...
    //通过默认的构造器来创建对象
    _webView = [[WKWebView alloc] initWithFrame:CGRectMake(0, NAVHEIGHT, SCREENWIDTH, SCREENHEIGHT - NAVHEIGHT-TABBARHEIGHT)
...
//        StrongSelf
//        [JKEventHandler getInject:strongSelf.webView];
//        [strongSelf.webView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:strongSelf.url]]];
//    }];
...
didFailProvisionalNavigation:(null_unspecified WKNavigation *)navigation withError:(NSError *)error {
    ...
    myView = [[EmptyView alloc]initWithFrame:CGRectMake(0, NAVHEIGHT, 0, SCREENHEIGHT - NAVHEIGHT)];
    WeakSelf
    myView.freshBlock = ^{
        StrongSelf
        [strongSelf.webView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:strongSelf.url]]];
